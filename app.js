...
$(function()
{
	...
	}).on('click', '.btn-copy', function(e)
    {
        ...
        $('#functionCode').val("public function "+toCamelCase($name)+"(Request $request)\n    {\n        $limit = $request->limit ?? 15;\n        \n        $data = collect();\n        $excel = null;\n        return [\n            'data' => $data,\n            'excel' => $excel\n        ];\n    }\n");
    }).on('change','input[name="protected[]"]', onProtectedChanged
    ).on('change','input[name="protectedVal[]"]', onProtectedChanged
    ).on('change','input[name="fontVarName[]"]', onFontChanged
    ...
    ).on('change','input[name="fillVarName[]"]', onFillChanged
...
function onFontChanged() {
    // Protecteds
    var varNames = $("input[name='fontVarName[]']")
        .map(function(){return $(this).val();}).get();
    ...
    names.forEach((name, count) => {
        ...
        if (varNames[count] === '') {
            varNames[count] = ($colorsArray['#'.concat(color)]) ? $colorsArray['#'.concat(color)] : color ;
            $.each($('input[name="fontVarName[]"]'), function (key) {
                if (key === count) {
                    $(this).val(varNames[key]);
                }
            })
        }
    });
    names.forEach((name, count) => {
        var varName = varNames[count];
        ...
        if (name.length > 0) {
            ...
            if (varName==='' && color !== '000000')
                var_name+=toPascalCase(color);
            else
                var_name+=toPascalCase(varName);
            if (bold)
                var_name += 'Bold';
            if (underline)
                var_name += 'Underline';
...
function onFillChanged() {
    // Protecteds
    var fillNames = $("input[name='fillVarName[]']")
        .map(function(){return $(this).val();}).get();
    ...
    types.forEach((type, count) => {
        var color = colors[count].replace('#','');
        if (fillNames[count] === '') {
            fillNames[count] = ($colorsArray['#'.concat(color)]) ? $colorsArray['#'.concat(color)] : color ;
            $.each($('input[name="fillVarName[]"]'), function (key) {
                if (key === count) {
                    $(this).val(fillNames[key]);
                }
            })
        }
    });
    types.forEach((type, count) => {
        var fillName = fillNames[count]
        var color = colors[count].replace('#','');
        if (type.length > 0 && type != 'none') {
            let var_name = toPascalCase(type)
            var_name+=toPascalCase(fillName);
